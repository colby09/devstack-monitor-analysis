...
import { Download, Play, RefreshCw, FileText, AlertTriangle, CheckCircle, FileSearch, BarChart3, HardDrive } from 'lucide-react';
...
const ForensicsPage: React.FC = () => {
  ...
  // Fetch memory dumps
  const fetchMemoryDumps = async () => {
    try {
      const response = await fetch('/api/dumps');
      if (response.ok) {
        const data = await response.json();
        // Filter only completed dumps for analysis (check for both uppercase and lowercase)
        const completedDumps = data.filter((dump: MemoryDump) => 
          dump.status === 'COMPLETED' || dump.status === 'completed'
        );
        setMemoryDumps(completedDumps);
        console.log('Fetched memory dumps:', completedDumps); // Debug log
      }
    } catch (error) {
  ...
  // Start analysis from existing dump
  const startAnalysisFromDump = async () => {
    if (!selectedDump) return;
    
    const dump = memoryDumps.find(d => d.id === selectedDump);
    if (!dump) return;

    setDumpLoading(true);
    try {
      const response = await fetch('/api/integrated-forensic/start-from-dump', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (response.ok) {
        const data = await response.json();
        console.log('Analysis started from dump:', data);
        await fetchAnalyses();
        setSelectedDump(''); // Reset selection
      } else {
        const error = await response.json();
        console.error('Failed to start analysis from dump:', error);
      }
    } catch (error) {
      console.error('Failed to start analysis from dump:', error);
    } finally {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setRefreshing(true);
      Promise.all([fetchMemoryDumps(), fetchAnalyses()])
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
      ...
      {/* Main Analysis Tabs */}
      <Tabs defaultValue="dump-first" className="w-full">
        <TabsList className="grid w-full grid-cols-2">
          <TabsTrigger value="dump-first" className="flex items-center gap-2">
            <HardDrive className="h-4 w-4" />
            Analyze Existing Dumps
          </TabsTrigger>
          <TabsTrigger value="instance-first" className="flex items-center gap-2">
        ...
        {/* Dump-First Tab */}
        <TabsContent value="dump-first" className="space-y-6">
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
              ...
              <CardDescription>
                Select a completed memory dump to start forensic analysis
              </CardDescription>
            </CardHeader>
            <CardContent>
              {memoryDumps.length === 0 ? (
                <Alert>
                  ...
                  <AlertDescription>
                    No completed memory dumps available. Create memory dumps first in the "Dumped Ram" tab.
                  </AlertDescription>
                </Alert>
              ) : (
                <div className="space-y-4">
                  <div className="flex gap-4 items-end">
                    <div className="flex-1">
                      <label className="block text-sm font-medium mb-2">Select Memory Dump</label>
                      <select
                        value={selectedDump}
                      ...
                      >
                        <option value="">Choose a memory dump...</option>
                        {memoryDumps.map((dump) => (
                          <option key={dump.id} value={dump.id}>
                            {dump.instance_name} - {formatFileSize(dump.file_size)} - {new Date(dump.created_at).toLocaleString()}
                          </option>
                        ))}
                      </select>
                    </div>
                    <Button
                      onClick={startAnalysisFromDump}
                      disabled={!selectedDump || dumpLoading}
                      ...
                      {(() => {
                        ...
                        return (
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                            <div>
                              <div className="font-medium text-gray-600">Instance</div>
                              <div className="font-semibold">{dump.instance_name}</div>
                            </div>
                            <div>
                              <div className="font-medium text-gray-600">OS Type</div>
                              <div className="font-semibold">{dump.os_type}</div>
                            </div>
                            <div>
                              <div className="font-medium text-gray-600">File Size</div>
                              <div className="font-semibold">{formatFileSize(dump.file_size)}</div>
                            </div>
                            <div>
                              <div className="font-medium text-gray-600">Created</div>
                              <div className="font-semibold">{new Date(dump.created_at).toLocaleString()}</div>
                            </div>
                          </div>
                        );
                      })()}
        ...
        {/* Instance-First Tab */}
        <TabsContent value="instance-first" className="space-y-6">
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
              ...
              <CardDescription>
                Select an instance to perform complete forensic analysis (dump + analysis)
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="flex gap-4 items-end">
